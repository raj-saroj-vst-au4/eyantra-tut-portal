...
import { signOutWithKeycloak } from "../utils/auth";
...
export default function Sidebar({ children }: Props) {
  ...
  return (
    <>
      ...
      <aside
        id="logo-sidebar"
        className={`flex flex-col justify-between fixed top-0 left-0 z-40 w-64 h-screen transition-transform ${
          showNav ? "transform-none" : "-translate-x-full"
        } sm:translate-x-0`}
        aria-label="Sidebar"
      >
        ...
        <div className="flex justify-between items-center w-full px-4 py-7 bottom-2 bg-gray-800">
          ...
          <button className="px-4" onClick={() => signOutWithKeycloak()}>
            <GoSignOut />
          </button>
        </div>
      </aside>
        ...
        <div className="p-4 border-2 border-dashed rounded-lg border-gray-700">
          {showNav && (
            <div
              ...
              onClick={() => setShowNav(false)} // Close sidebar when clicking the backdrop
            />
          )}
          {children}
        </div>
